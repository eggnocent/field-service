pipeline {
  ...
  stages {
    ...
    stage('Checkout Code') {
      steps {
        script {
          def repoUrl = 'https://github.com/eggnocent/field-service.git'
    ...
    stage('Commit and Push Changes') {
      steps {
        script {
          ...
          git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@github.com/eggnocent/field-service.git
            ...
            withCredentials([
                ...
                sh '''
                    ...
                    ssh -o StrictHostKeyChecking=no -i "$SSH_KEY_FILE" ${SSH_USER}@${HOST} bash <<'ENDSSH'
                        ...
                        TARGET_DIR="/home/eggnocent/mini-soccer-project/field-service"
                        ...
                        if [ -d "$TARGET_DIR/.git" ]; then
                            ...
                            git clone -b "''' + targetBranch + '''" https://github.com/eggnocent/field-service.git "$TARGET_DIR"
                            cd "$TARGET_DIR"
                        fi
                        ...
                        sed -i "s|^CONSUL_HTTP_PATH=.*|CONSUL_HTTP_PATH=backend\\/field-service|" .env
